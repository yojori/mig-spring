...
<%  
    ...
    if ("insertAll".equals(search.getMode())) {
    	String[] insert_table_seq = request.getParameterValues("insert_table_seq");
    	String[] source_table = request.getParameterValues("source_table");
    	String[] source_pk = request.getParameterValues("source_pk");
    	String[] target_table = request.getParameterValues("target_table");
    	
    	for (int i = 0; i < insert_table_seq.length; i++) {
            ...
            String truncate_yn = request.getParameter("truncate_" + insert_table_seq[i]);
            
            if (truncate_yn != null && "Y".equals(truncate_yn)) {
            	entity.setTruncate_yn("Y");	
            }
    	    
    	    if (source_table[i] == null || source_table[i].length() == 0) {
    	    	continue;
    	    } else if (insert_table_seq[i] == null || insert_table_seq[i].length() == 0) { // insert
    	    ...
    	    } else if (insert_table_seq[i] != null && insert_table_seq[i].length() > 0) { // update
