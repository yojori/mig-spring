    ...
    <div class="container-fluid">
        ...
        <div class="collapse navbar-collapse" id="navbarNav">
            <ul class="navbar-nav ms-auto">
                ...
                <li class="nav-item"><a class="nav-link active fw-bold text-warning" href="./migration-work-list.jsp">이관 진행 현황</a></li>
    ...
    <form name="frm1" method="get">
                ...
                <div class="card p-2 d-inline-block bg-white">
                    <div class="d-flex align-items-center">
                        ...
                        <select name="status" class="form-select form-select-sm me-2" style="width: 120px;">
                            ...
                            <option value="READY" ${search.status == 'READY' ? 'selected' : ''}>대기중 (READY)</option>
                            <option value="RUNNING" ${search.status == 'RUNNING' ? 'selected' : ''}>진행중 (RUNNING)</option>
                            <option value="DONE" ${search.status == 'DONE' ? 'selected' : ''}>완료 (DONE)</option>
                            <option value="FAIL" ${search.status == 'FAIL' ? 'selected' : ''}>실패 (FAIL)</option>
                        </select>
        ...
        <div class="card p-3">
                    ...
                    <tbody>
                        <c:forEach items="${list}" var="list">
                            <tr>
                                ...
                                <td>
                                    <span class="fw-bold text-primary">${list.mig_name}</span>
                                    <span class="text-muted small">(${list.mig_list_seq})</span>
                                </td>
                                <td class="text-center small text-muted">${list.worker_id}</td>
                                <td class="text-center">
                                    <c:choose>
                                        <c:when test="${list.status eq 'READY'}"><span class="badge bg-secondary status-badge">READY</span></c:when>
                                        <c:when test="${list.status eq 'RUNNING'}"><span class="badge bg-primary status-badge">RUNNING</span></c:when>
                                        <c:when test="${list.status eq 'DONE'}"><span class="badge bg-success status-badge">DONE</span></c:when>
                                        <c:when test="${list.status eq 'FAIL'}"><span class="badge bg-danger status-badge">FAIL</span></c:when>
                                ...
                                <td class="text-center small text-muted"><fmt:formatDate value="${list.create_date}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
                            </tr>
                        </c:forEach>
            ...
            <div class="d-flex justify-content-center mt-4">
                 <jaes:paging totalCount="${search.totalCount}" linkUrl="./migration-work-list.jsp?status=${search.status}" pageSize="${search.pageSize}" skin="front" currentPage="${search.currentPage}" prefix="search" />
            </div>
        </div>
    </form>
